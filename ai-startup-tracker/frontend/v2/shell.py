...
def is_active() -> bool:
    """This shell is the site now, so it runs unless the old one is asked for.

    ?v=1 still reaches the previous shell, which is worth keeping while the two
    are being compared and while anything links to it.
    """
    return st.query_params.get(QUERY_FLAG) != LEGACY_VALUE
...
def leave() -> None:
    ...
    This has to set the flag rather than clear it: with no flag present this
    shell is the default, so clearing would land straight back here.
    """
    st.query_params[QUERY_FLAG] = LEGACY_VALUE
    st.rerun()
